            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-20">
                {hasFilters ? (
                    ...
                    <>
                        ...
                        <p className="text-muted-foreground">لا توجد بلاغات بعد.</p>
                        <p className="text-sm text-muted-foreground/60">عندما يرسل الطلاب بلاغات ستظهر هنا.</p>
                    </>
                )}
            </div>
        );
            ...
            <table className="w-full text-sm">
                <thead className="sticky top-0 z-10">
                    <tr className="border-b bg-muted/80 text-start backdrop-blur-sm">
                        <SortHeader sort={sort} direction={direction} field="id" label="#" className="w-16" />
                        <SortHeader sort={sort} direction={direction} field="type" label="النوع" />
                        <th className="px-4 py-3 font-medium whitespace-nowrap">المرسل</th>
                        <th className="px-4 py-3 font-medium whitespace-nowrap">السؤال</th>
                        <SortHeader sort={sort} direction={direction} field="status" label="الحالة" />
                        <SortHeader sort={sort} direction={direction} field="created_at" label="التاريخ" />
                        ...
                        >
                            ...
                            <td className="px-4 py-3">
                                <span className="inline-flex items-center gap-1.5 text-sm">
                                    <span>{reportTypeMeta[report.type].emoji}</span>
                                    <span>{reportTypeMeta[report.type].label}</span>
                                </span>
                            </td>
                            ...
                            <td className="max-w-[200px] truncate px-4 py-3 text-muted-foreground">
                                {report.question ? (
                                    <span className="text-xs">{report.question.text}</span>
                                ) : (
                                    <span className="text-xs text-muted-foreground/50">—</span>
                                )}
                            </td>
                            <td className="px-4 py-3">
                                <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-bold', reportStatusMeta[report.status].bgClass)}>
                                    <span className={cn('h-1.5 w-1.5 rounded-full', reportStatusMeta[report.status].dotClass)} />
