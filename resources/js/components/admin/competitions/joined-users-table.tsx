...
export default function JoinedUsersTable({
    users,
    meta,
    competitionName,
}: JoinedUsersTableProps) {
    if (users.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-16">
                <Users className="h-10 w-10 text-muted-foreground/30" />
                <p className="text-muted-foreground">
                    لا يوجد طلاب منضمون بعد.
                </p>
                <p className="text-sm text-muted-foreground/60">
                    يمكن للطلاب الانضمام إلى "{competitionName}" من صفحة
                    المسابقات.
                </p>
            </div>
        );
            ...
            <table className="w-full text-sm">
                <thead className="sticky top-0 z-10">
                    <tr className="border-b bg-muted/80 text-start backdrop-blur-sm">
                        <th className="w-12 px-4 py-3 text-center font-medium whitespace-nowrap">
                            #
                        </th>
