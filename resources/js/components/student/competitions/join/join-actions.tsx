...
export default function JoinActions({
    ...
    const handleJoin = () => {
        setLoading(true);
        router.post(
            join.store.url({ competition: competition.slug }),
            {},
            {
        ...
        return (
            <div className="flex flex-col items-center gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 sm:flex-row">
                <p className="grow text-center text-sm font-bold text-emerald-800 sm:text-right">
                    <span aria-hidden="true">🎉</span> أنت منضم بالفعل إلى هذه
                    المسابقة، يمكنك الانتقال مباشرة لبدء الاختبارات!
                </p>
                <Link
    ...
    return (
        ...
        >
            {loading
                ? 'جاري تسجيل انضمامك...'
                : 'تأكيد الانضمام للمسابقة وبدء التحدي'}
        </Button>
    );
}
