...
export function RechartsBar({ data, height = 240 }: { data: ChartData[]; height?: number }) {
    ...
    return (
        <svg width="100%" height={height} viewBox={`0 0 ${w} ${height}`} preserveAspectRatio="xMidYMid meet">
                ...
                return (
                    <g key={tick}>
                        <line
                            ...
                            strokeDasharray="3 3"
                        />
                        <text x={MARGIN.left + chartW + 4} y={y + 3} textAnchor="start" fontSize={10} className="fill-muted-foreground/60">
                            {tick}%
                        </text>
                    </g>
                );
            ...
            {/* X-axis labels */}
            {points.map((p, i) => (
                <text key={`x-${i}`} x={p.x} y={height - MARGIN.bottom + 12} textAnchor="middle" fontSize={10} className="fill-muted-foreground/60">
                    {i + 1}
                </text>
            ))}
            <text x={MARGIN.left + chartW / 2} y={height - 4} textAnchor="middle" fontSize={9} className="fill-muted-foreground/40">
                المحاولة
            </text>
