...
export function ExamHeader({
    ...
    const progressPct =
        totalMinutes > 0 ? (elapsedSeconds / totalSeconds) * 100 : 0;
    ...
    return (
        <header
            className="sticky top-0 z-50 border-b bg-background shadow-xs"
            role="banner"
            aria-label="شريط معلومات الاختبار"
        >
            <div className="relative mx-auto flex h-14 max-w-4xl items-center justify-between px-4">
                ...
                <div className="flex items-center gap-3">
                    <Badge
                        variant={type === 'exam' ? 'destructive' : 'secondary'}
                        className="shrink-0"
                    >
                        {type === 'exam' ? (
                            <>
                    ...
                    {totalMinutes > 0 && (
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                <span
                                    className={`absolute text-[10px] font-bold tabular-nums ${isCritical ? 'text-destructive' : isUrgent ? 'text-warning' : 'text-muted-foreground'}`}
                                >
                                    {remainingMinutes}
                                </span>
                            </div>
                            <span
                                className={`hidden items-center gap-1.5 text-sm font-medium tabular-nums sm:flex ${isCritical ? 'animate-pulse text-destructive' : isUrgent ? 'text-warning' : 'text-muted-foreground'}`}
                                aria-live="polite"
                                aria-atomic="true"
                            >
                                {timerDisplay}
                            ...
                            <span
                                className={`text-xs font-bold tabular-nums sm:hidden ${isCritical ? 'text-destructive' : isUrgent ? 'text-warning' : 'text-muted-foreground'}`}
                            >
                                {timerDisplay}
                            </span>
                        </div>
