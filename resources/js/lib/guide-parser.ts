import type { CalloutData, FaqItem, GuideDocument, ParsedSection, RelatedArticle, Source, TableData, TimelineStep } from '@/types/guide';
...
function parseCallouts(lines: string[]): { callouts: CalloutData[]; remaining: string[] } {
...
break;
 ...
 lines2[lines2.length - 1] += ' ' + sub; 
                ...
                if (/قانون/i.test(label) && /مادة/i.test(msg)) {
                    ...
                    callouts.push({ type: 'warning', title: label, message: msg });
...
function classifySection(title: string): { type: ParsedSection['type']; cleanTitle: string } {
...
export function parseGuideDocument(markdown: string): GuideDocument {
    ...
    for (const raw of rawSections) {
        ...
        const lines = raw.lines.filter((l) => l.trim() !== '---' && l.trim() !== '');

        // ─── Hero ───
        if (normalizedTitle === 'hero') {
            const heroLines = lines.filter((l) => !l.trim().startsWith('#') && l.trim() !== '>');
            const blockquotes = lines.filter((l) => l.trim().startsWith('> '));
            const descLines = [...heroLines, ...blockquotes.map((l) => l.trim().slice(2).trim())];
            heroDescription = descLines.join(' ').replace(/\*\*(.+?)\*\*/g, '$1');
            continue;
        }
...
codeLines.push(cl);
                ...
                for (const cl of codeLines) {
                    ...
                    if (title) {
                        if (prevTitle) {
                            timelineSteps.push({ id: `step-${timelineSteps.length + 1}`, title: prevTitle, description: '' });
                        }
        ...
        if (normalizedTitle === 'main content') {
            ...
            for (const block of h3Blocks) {
                ...
                const id = `step-${titleClean.replace(/[^\w\u0600-\u06FF\s-]/g, '').replace(/\s+/g, '-').toLowerCase()}`;
...
section.callouts = callouts;
        ...
        if (type === 'cta') {
            ...
            const buttonMatch = lines.map((l) => l.trim()).find((l) => l.startsWith('[') && l.includes(']('));
            ...
            sections.push({
                ...
                number: sectionCounter,
                cta: {
                    heading: headingLine ? headingLine.trim().replace(/\*\*/g, '') : 'استعد الآن',
...
return null;
                ...
                .filter(Boolean) as { law: string; article: string; summary: string }[];
        ...
        sectionCounter++;
        const cleanId = raw.title.replace(/\s+/g, '-').replace(/[^\w\u0600-\u06FF\s-]/g, '').replace(/\s+/g, '-').toLowerCase();
...
fallbackSection.callouts = callouts;
    ...
    const effectiveTimeline = timelineSteps.length > 0
        ? timelineSteps
