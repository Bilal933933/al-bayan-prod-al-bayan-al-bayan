...
import { BookOpen, ChartNoAxesColumn, ChevronLeft, Clock, House, Play, RotateCcw, Sparkles, Target, Trophy } from 'lucide-react';
    ...
    return (
        <>
            ...
            <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="mx-auto max-w-4xl px-4 pb-12 pt-6 sm:px-6"
            >
                {/* Breadcrumb */}
                ...
                {/* Hero Card */}
                <div className={cn('overflow-hidden rounded-2xl border bg-card shadow-sm', color.border)}>
                    <div className={cn('bg-gradient-to-br p-6 sm:p-8', color.from)}>
                        ...
                        <div className="mt-4 flex items-start gap-4">
                            <div className={cn('flex h-14 w-14 shrink-0 items-center justify-center rounded-xl', color.bg)}>
                                <BookOpen className={cn('h-7 w-7', color.icon)} />
                            </div>
                            <div className="min-w-0 flex-1">
                                <h1 className="text-2xl font-black sm:text-3xl">{topic.name}</h1>
                    ...
                    <div className="flex flex-wrap items-center gap-5 border-t px-6 py-4 sm:px-8">
                        <span className="inline-flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-1.5 text-sm text-muted-foreground">
                            ...
                            <span className="font-semibold">{topic.default_duration_minutes ? `${topic.default_duration_minutes} دقيقة` : 'بدون مؤقت'}</span>
                        </span>
                        <span className="inline-flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-1.5 text-sm text-muted-foreground">
                            <BookOpen className="h-4 w-4 text-muted-foreground/60" />
                            <span className="font-semibold">{questionsCount} سؤال</span>
                        </span>
                        {bestScorePercent !== null && (
                            <span className="inline-flex items-center gap-2 rounded-lg bg-amber-50 px-3 py-1.5 text-sm text-amber-700">
                                <Trophy className="h-4 w-4" />
                                <span className="font-semibold">أفضل نتيجة: {userStats.best_score!.correct_answers}/{userStats.best_score!.total_questions} ({bestScorePercent}%)</span>
                            </span>
                        )}
                    </div>
                </div>
                ...
                {hasStats && (
                    <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-4">
                        <div className="rounded-2xl border bg-card p-5 text-center shadow-xs">
                            <p className="text-2xl font-black text-foreground">{userStats.total_attempts}</p>
                            <p className="mt-1 text-xs font-medium text-muted-foreground">إجمالي المحاولات</p>
                        </div>
                        {userStats.average_percentage !== null && (
                            <div className="rounded-2xl border bg-card p-5 text-center shadow-xs">
                                <p className="text-2xl font-black text-foreground">{userStats.average_percentage}%</p>
                                <p className="mt-1 text-xs font-medium text-muted-foreground">متوسط النتيجة</p>
                            </div>
                        )}
                        {bestScorePercent !== null && (
                            <div className="rounded-2xl border bg-card p-5 text-center shadow-xs">
                                <p className="text-2xl font-black text-foreground">{userStats.best_score!.correct_answers}/{userStats.best_score!.total_questions}</p>
                                <p className="mt-1 text-xs font-medium text-muted-foreground">أفضل نتيجة</p>
                            </div>
                        )}
                ...
                <div className="mt-6 rounded-2xl border bg-card p-6 shadow-xs sm:p-8">
                    ...
                    <p className="text-sm leading-relaxed text-muted-foreground">
                        راجع القواعد الأساسية للكسور والنسب أولاً قبل الانتقال للمسائل المركبة، وتدرّب على حل الأسئلة بدون آلة حاسبة لتحسين سرعتك، مع التركيز على الأسئلة متوسطة الصعوبة لأنها الأكثر تكرارًا في الاختبار الفعلي.
                    </p>
                </div>
                ...
                {questionsCount > 0 && (
                    <div className="mt-6 rounded-2xl border bg-card p-6 shadow-xs sm:p-8">
                        ...
                        <TopicDifficultyBars distribution={difficultyDistribution} />
                    </div>
                )}
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
                    {hasInProgress ? (
                        <>
                            <Button onClick={handleResume} size="lg" className="gap-2 text-base shadow-lg">
                        ...
                        <Button onClick={handleStart} size="lg" className="gap-2 text-base shadow-lg">
                            <Play className="h-5 w-5" />
                            بدء التدريب
                        </Button>
                    )}
